...
def main():
    ...
    graph, stations = create_graph('london_underground_graph by Group 38.csv')

    # Asking the user to enter his start and his destination
    start, end = input("Enter start station: "), input("Enter end station: ")

    start_time = time.time_ns()  # Start timing after user input

    # Finding the shortest path between the start and end
    path, travel_time = find_shortest_path(graph, stations, start, end)

    end_time = time.time_ns()  # End timing

    # Showing the path and total travel time
    print(f"Path: {' -> '.join(path)}\nDuration: {travel_time} minutes")

    processing_time_ns = end_time - start_time  # Calculate processing time in nanoseconds
    print(f"Processing Time: {processing_time_ns / 1e6} milliseconds")  # Convert to milliseconds and print
